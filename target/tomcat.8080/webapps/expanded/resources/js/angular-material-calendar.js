...
angular.module("materialCalendar").config(
		[ "materialCalendar.config", "$logProvider", "$compileProvider",
...
angular.module("materialCalendar").directive('compile',
...
angular.module("materialCalendar").service(
		...
		[ function() {
			...
			function Calendar(year, month, options) {
				...
				this.weeks = [];
				this.weekStartsOn = this
						.setWeekStartsOn(this.options.weekStartsOn);
				this.startDateOfMonth = this
						.setStartDateOfMonth(this.options.startDateOfMonth);
				this.noOfDays = this.setNoOfDays(this.options.noOfDays);

				this.next = function() {
					if (this.start.getMonth() < 11) {
						this.init(this.start.getFullYear(), this.start
								.getMonth() + 1);
						return;
					}
				...
				// Month should be the javascript indexed month, 0 is January,
				// etc.
				this.init = function(year, month) {

					var now = new Date();
					this.year = angular.isDefined(year) ? year : now
							.getFullYear();
					this.month = angular.isDefined(month) ? month : now
							.getMonth();
					...
					for (var i = 0; i < monthLength; ++i) {
						...
						this.weeks[this.weeks.length - 1].push(angular
								.copy(date));
...
angular
		...
		.directive(
				...
				[
						...
						"materialCalendar.Calendar",
						"MaterialCalendarData",
						function($compile, $parse, $templateRequest, $q,
							...
							return {
								...
								link : function($scope, $element, $attrs) {
									...
									var date = new Date();
									var month = parseInt($attrs.startMonth
											|| date.getMonth());
									var year = parseInt($attrs.startYear
											|| date.getFullYear());
									...
									// Parse the parent model to determine if
									// it's an array.
									// If it is an array, than we'll
									// automatically be able to select
									// more than one date.
									if ($attrs.ngModel) {
									...
									$scope.titleFormat = $scope.titleFormat
											|| "LLLL yyyy";
									$scope.dayLabelFormat = $scope.dayLabelFormat
											|| "EEE";
									$scope.dayLabelTooltipFormat = $scope.dayLabelTooltipFormat
											|| "EEEE";
									$scope.dayFormat = $scope.dayFormat || "d";
									$scope.dayTooltipFormat = $scope.dayTooltipFormat
											|| "fullDate";
									...
									$scope.isDisabled = function(date,
											startDateOfMonth, noOfDays) {
										if (noOfDays != 0
												&& angular.isDefined(noOfDays)) {
											...
											dateEnd.setDate(dateStart.getDate()
													+ parseInt(noOfDays));
											if (date.getDate() <= dateStart
													&& date.getDate() >= dateEnd) {
												return true;
											}
										}
										...
										if ($scope.disableFutureSelection
												&& date > new Date()) {
											return true;
										}
									...
									$scope.calendarDirection = $scope.calendarDirection
											|| "horizontal";
									...
									$scope
											.$watch(
													"calendarDirection",
													function(val) {
														$scope.weekLayout = val === "horizontal" ? "row"
																: "column";
													});
									...
									$scope.handleDayClick = function(date) {
										...
										handleCb($scope.onDayClick, angular
												.copy(date));

									};

									// Small helper function to set the contents
									// of the template.
									...
									var init = function() {
										...
										// Allows fetching of dynamic templates
										// via $templateCache.
										if ($scope.templateUrl) {
											return $templateRequest($scope.templateUrl);
										}

										return $q.resolve($scope.template()
												|| defaultTemplate);
									};
									...
									var getDayContent = function(date) {
										...
										// Initialize the data in the data
										// array.
										if ($scope.noCache) {
											$scope.dataService.setDayContent(
													date, "");
										} else {
											$scope.dataService
													.setDayContent(
															date,
															($scope.dataService.data[getDayKey(date)] || ""));
										}
										...
										// Check for async function. This should
										// support $http.get() and also regular
										// $q.defer() functions.
										if (angular.isObject(result)
												&& "function" === typeof result.success) {
									...
									var setData = function() {
										angular.forEach($scope.calendar.weeks,
												function(week) {
													angular.forEach(week,
															getDayContent);
												});
									};
